...
returned by `readGMT()` is a `GeneSetCollection` object defined on that package.
...
later one allows other software to figure out what kind of gene identifiers are
...
database that happens to have a duplicated gene set name and which we have
...
not happen with `readGMT()` and that, by default, the first occurrence of the
duplicated gene set has been removed.
...
inluding the addition of metadata that enables an automatic mapping of the gene
